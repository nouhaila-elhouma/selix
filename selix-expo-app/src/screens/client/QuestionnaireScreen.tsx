...
import { FinancingMode, LeadAnswers, Objective, PropertyType, PurchaseDeadline } from '../../types';

const TYPES: PropertyType[] = ['Appartement', 'Villa', 'Bureau', 'Terrain', 'Duplex', 'Penthouse', 'Studio', 'Local', 'Riad'];
const GOALS: Array<{ label: string; value: Objective | '' }> = [{ label: 'Habiter', value: 'Habiter' }, { label: 'Investir', value: 'Investir' }, { label: 'Autre', value: '' }];
const MONEY: Array<{ label: string; value: FinancingMode | '' }> = [{ label: 'Cash', value: 'Cash' }, { label: 'Credit', value: 'Crédit' }, { label: 'En cours', value: 'Mixte' }];
const DEADLINES: Array<{ label: string; value: PurchaseDeadline }> = [{ label: 'Immediat', value: 'Immédiat' }, { label: '1 a 3 mois', value: '3 mois' }, { label: '3 a 6 mois', value: '6 mois' }, { label: 'Plus tard', value: 'Plus de 1 an' }];
  ...
  return (
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        {step === 0 ? <><View style={styles.chips}>{GOALS.map((item) => <SelectChip key={item.label} label={item.label} selected={form.objective === item.value || (!item.value && !form.objective)} onPress={() => { setValue('objective', item.value); if (!item.value) setValue('clientGoal', 'Residence secondaire'); }} />)}</View>{!form.objective ? <Input label="Precisez votre objectif" value={form.clientGoal || ''} onChangeText={(v) => setValue('clientGoal', v)} placeholder="Habiter, residence secondaire, location..." icon="create-outline" /> : null}</> : null}
        {step === 1 ? choiceGroup(TYPES, form.propertyType, (v) => setValue('propertyType', v)) : null}
        {step === 2 ? <><Input label="Ville recherchee" value={form.searchedCity || ''} onChangeText={(v) => setValue('searchedCity', v)} placeholder="Casablanca, Rabat, Marrakech..." icon="navigate-outline" /><Input label="Zone / quartier" value={form.targetZone || ''} onChangeText={(v) => setValue('targetZone', v)} placeholder="Ain Diab, Agdal, Gueliz..." icon="map-outline" /><Input label="Zones refusees" value={(form.excludedZones || []).join(', ')} onChangeText={(v) => setValue('excludedZones', parseCsv(v))} placeholder="Ex : centre-ville, zone industrielle" icon="close-circle-outline" /><Input label="Ville actuelle" value={form.currentCity || ''} onChangeText={(v) => setValue('currentCity', v)} placeholder="Votre ville actuelle" icon="location-outline" /></> : null}
        {step === 3 ? <><View style={styles.row}><Input style={styles.half} label="Budget min (MAD)" value={form.budgetMinRaw ? String(form.budgetMinRaw) : ''} onChangeText={(v) => setValue('budgetMinRaw', parseNumber(v))} placeholder="1200000" icon="cash-outline" keyboardType="numeric" /><Input style={styles.half} label="Budget max (MAD)" value={form.budgetRaw ? String(form.budgetRaw) : ''} onChangeText={(v) => setValue('budgetRaw', parseNumber(v))} placeholder="1800000" icon="cash-outline" keyboardType="numeric" /></View><Input label="Apport disponible (MAD)" value={form.downPaymentRaw ? String(form.downPaymentRaw) : ''} onChangeText={(v) => setValue('downPaymentRaw', parseNumber(v))} placeholder="300000" icon="wallet-outline" keyboardType="numeric" /><Text style={styles.label}>Financement</Text>{choiceGroup(MONEY.map((item) => item.label), MONEY.find((item) => item.value === form.financing)?.label || '', (v) => setValue('financing', MONEY.find((item) => item.label === v)?.value || ''))}</> : null}
        {step === 4 ? <><View style={styles.row}><Input style={styles.half} label="Chambres" value={form.rooms ? String(form.rooms) : ''} onChangeText={(v) => setValue('rooms', parseNumber(v))} placeholder="3" icon="bed-outline" keyboardType="numeric" /><Input style={styles.half} label="Surface ideale (m2)" value={form.desiredAreaRaw ? String(form.desiredAreaRaw) : ''} onChangeText={(v) => setValue('desiredAreaRaw', parseNumber(v))} placeholder="120" icon="resize-outline" keyboardType="numeric" /></View><Text style={styles.label}>Options importantes</Text><View style={styles.chips}>{MUST.map((item) => <SelectChip key={item} label={item} selected={(form.mustHave || []).includes(item)} onPress={() => toggleMust(item)} />)}</View><Text style={styles.label}>Parking obligatoire ?</Text>{booleanGroup('parkingRequired')}<Text style={styles.label}>Balcon / terrasse obligatoire ?</Text>{booleanGroup('terraceRequired')}</> : null}
        {step === 5 ? <><Text style={styles.label}>Quand voulez-vous acheter ?</Text>{choiceGroup(DEADLINES.map((item) => item.label), DEADLINES.find((item) => item.value === form.purchaseDeadline)?.label || '', (v) => setValue('purchaseDeadline', DEADLINES.find((item) => item.label === v)?.value || ''))}<Text style={styles.label}>Avez-vous deja visite des biens ?</Text>{booleanGroup('hasVisitedProperties')}<Text style={styles.label}>Projet pret a livrer ou en construction ?</Text>{choiceGroup(['Pret a livrer', 'En cours de construction', 'Sans preference'], form.projectStage || '', (v) => setValue('projectStage', v))}</> : null}
        {step === 6 ? <><Text style={styles.label}>Avez-vous deja contacte une banque ?</Text>{booleanGroup('contactedBank')}<Text style={styles.label}>Avez-vous une pre-approbation bancaire ?</Text>{booleanGroup('hasBankPreApproval')}<Text style={styles.label}>Etes-vous decisionnaire ?</Text>{booleanGroup('isDecisionMaker')}<Input label="Remarques libres" value={form.freeNotes || ''} onChangeText={(v) => setValue('freeNotes', v)} placeholder="Precisez ce qui compte vraiment pour vous" icon="document-text-outline" multiline /></> : null}
        {step === 7 ? <><View style={styles.result}><Text style={styles.resultTitle}>{isGuest ? 'Vos resultats sont prets, creez votre compte pour les voir' : 'Votre projet est pret a etre relance'}</Text><Text style={styles.resultText}>Selix a deja calcule votre maturite, votre coherence budget / marche et votre potentiel de matching.</Text></View>{isGuest ? <><Input label="Prenom" value={form.firstName || ''} onChangeText={(v) => setValue('firstName', v)} placeholder="Votre prenom" icon="person-outline" /><Input label="Nom" value={form.lastName || ''} onChangeText={(v) => setValue('lastName', v)} placeholder="Votre nom" icon="person-outline" /><Input label="Telephone" value={form.phone || ''} onChangeText={(v) => setValue('phone', v)} placeholder="+212 6XX XXX XXX" icon="call-outline" keyboardType="phone-pad" /><Input label="Email" value={form.email || ''} onChangeText={(v) => setValue('email', v)} placeholder="votre@email.com" icon="mail-outline" keyboardType="email-address" /><View style={styles.passwordWrap}><Input label="Mot de passe" value={form.password || ''} onChangeText={(v) => setValue('password', v)} placeholder="Au moins 6 caracteres" icon="lock-closed-outline" secure={!showPassword} /><TouchableOpacity onPress={() => setShowPassword((prev) => !prev)} style={styles.eyeBtn}><Ionicons name={showPassword ? 'eye-outline' : 'eye-off-outline'} size={18} color={Colors.textSoft} /></TouchableOpacity></View><Text style={styles.label}>Langue preferee</Text>{choiceGroup(LANGUAGES, form.preferredLanguage || '', (v) => setValue('preferredLanguage', v))}</> : null}<TouchableOpacity onPress={() => setValue('consent', !form.consent)} style={styles.checkRow}><View style={[styles.box, form.consent && styles.boxOn]}>{form.consent ? <Ionicons name="checkmark" size={14} color={Colors.white} /> : null}</View><Text style={styles.checkText}>J accepte d etre contacte par Selix au sujet de mon projet immobilier.</Text></TouchableOpacity><TouchableOpacity onPress={() => setValue('contactWhatsApp', !form.contactWhatsApp)} style={styles.checkRow}><View style={[styles.box, form.contactWhatsApp && styles.boxOn]}>{form.contactWhatsApp ? <Ionicons name="checkmark" size={14} color={Colors.white} /> : null}</View><Text style={styles.checkText}>J accepte les echanges WhatsApp pour accelerer le suivi.</Text></TouchableOpacity>{error ? <View style={styles.error}><Ionicons name="alert-circle-outline" size={16} color={Colors.danger} /><Text style={styles.errorText}>{error}</Text></View> : null}</> : null}
      </ScrollView>
